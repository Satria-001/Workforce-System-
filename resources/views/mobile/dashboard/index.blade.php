...
@section('content')
    @php
        $karyawan = $karyawan ?? (object) [
            'nama_karyawan' => auth()->user()->name ?? 'Karyawan',
            'nama_jabatan' => 'Karyawan',
            'nama_dept' => '-',
            'foto' => null,
            'nik' => auth()->id() ?? 0,
        ];
        $namasettings = $namasettings ?? (object) ['mobile_theme_scheme' => 'green'];
        $pengumuman = $pengumuman ?? null;
        $notif_kontrak = $notif_kontrak ?? null;
        $notif_sp = $notif_sp ?? null;
        $presensihariini = $presensihariini ?? null;
        $presensi = $presensi ?? $presensihariini;
        $rekappresensi = $rekappresensi ?? null;
        $datapresensi = $datapresensi ?? [];
        $notiflembur = $notiflembur ?? 0;
        $lembur = $lembur ?? [];
        $denda_list = $denda_list ?? [];

        if (!function_exists('getNamaHari')) {
            function getNamaHari($day) { return $day; }
        }
        if (!function_exists('DateToIndo')) {
            function DateToIndo($date) { return $date; }
        }
        if (!function_exists('getfotoKaryawan')) {
            function getfotoKaryawan($foto) { return asset('images/user/owner.png'); }
        }
        if (!function_exists('hitungdenda')) {
            function hitungdenda($dendaList, $menitTelat) { return 0; }
        }
        if (!function_exists('hitungpulangcepat')) {
            function hitungpulangcepat(...$args) { return 0; }
        }
    @endphp
    ...
    <div id="header-section">
        ...
        <div id="section-user">
            <div id="user-info">
                <h3 id="user-name">{{ $karyawan->nama_karyawan ?? (auth()->user()->name ?? 'Karyawan') }}👋</h3>
                <span id="user-role">{{ $karyawan->nama_jabatan ?? 'Karyawan' }}</span>
                ...
                @if (!empty($karyawan->foto))
                    ...
                    @else
                        <div class="avatar avatar-xs me-2">
                            <img src="{{ asset('images/user/owner.png') }}" alt="avatar" class="imaged w64 rounded">
                        </div>
                    @endif
                @else
                    <div class="avatar avatar-xs me-2">
...
@push('myscript')
    ...
    <script type="text/javascript">
        ...
        $("#btnDaftarkanWajah").click(function(e) {
            e.preventDefault();
            window.location.href = "{{ route('app.attendance') }}";
        });
    </script>
@endpush
